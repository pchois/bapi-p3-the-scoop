...
function createComment(url, request) {
  ...
  if (requestComment && database.articles[requestComment.articleId] && database.users[requestComment.username]
      && requestComment.body) {
...
function deleteComment(url, request) {
  ...
  if(savedComment) {
    ...
    const usetCommentIds = database.users[savedComment.username].commentIds;
    usetCommentIds.splice(usetCommentIds.indexOf(id), 1);
...
function loadDatabase() {
  const databaseContent = yaml.safeLoad(fs.readFileSync('database.yaml', 'utf8'));
  database = databaseContent;
}

function saveDatabase() {
  const databaseContent = yaml.dump(database);
  fs.writeFileSync('database.yaml', databaseContent, 'utf8', function (err) {
    if (err) {
      console.log(err);
    }
    return 'DB was saved.';
